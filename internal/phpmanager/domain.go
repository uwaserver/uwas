// Per-domain PHP assignment and process lifecycle. Split out of
// manager.go per refactor.md A14: owns assigning PHP versions to domains
// (AssignDomain, RegisterExistingDomain), starting/stopping/restarting
// per-domain php-cgi subprocesses (StartDomain, StopDomain, RestartDomain)
// with crash-backoff auto-restart, per-domain php.ini overrides
// (SetDomainConfig, GetDomainConfig, buildDomainINI), and the domain
// lookup helpers used by the request hot path (RunningAddrForDomain).
package phpmanager
