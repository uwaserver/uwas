...
func TestEnsureDefaultConfig(t *testing.T) {
	...
	cfgPath, err := ensureDefaultConfig("8080", "9443")
...
func TestEnsureDefaultConfigIdempotent(t *testing.T) {
	...
	path1, err := ensureDefaultConfig("8080", "9443")
	...
	path2, err := ensureDefaultConfig("9999", "1111")
...
func TestGenerateDefaultConfigValidYAML(t *testing.T) {
	content := generateDefaultConfig("8080", "9443", "abcdef1234567890abcdef1234567890", "/tmp/uwas")
...
func TestGenerateDefaultConfigContainsAPIKey(t *testing.T) {
	...
	content := generateDefaultConfig("80", "9443", apiKey, "/tmp/uwas")
...
func TestGenerateDefaultConfigContainsPorts(t *testing.T) {
	content := generateDefaultConfig("8888", "7777", "key", "/tmp/uwas")
...
// ========== migrate.go internal parser tests (legacy code paths) ==========

func TestParseNginxConfigLegacy(t *testing.T) {
	// Test the legacy parseNginxConfig and convertNginxToYAML functions
	// that are still defined in migrate.go.
	content := `
server {
    listen 80;
...
`
	tmpFile, err := os.CreateTemp("", "nginx-legacy-*.conf")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(tmpFile.Name())
	...
	servers := parseNginxConfig(tmpFile)
	tmpFile.Close()

	if len(servers) != 1 {
		t.Fatalf("expected 1 server, got %d", len(servers))
	}
	if servers[0].serverName != "legacy.example.com" {
		t.Errorf("serverName = %q", servers[0].serverName)
	}
	if servers[0].root != "/var/www/legacy" {
		t.Errorf("root = %q", servers[0].root)
	}
}

func TestParseNginxConfigProxy(t *testing.T) {
	content := `
server {
    listen 80;
...
`
	tmpFile, err := os.CreateTemp("", "nginx-proxy-legacy-*.conf")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(tmpFile.Name())
	...
	if len(servers[0].locations) != 1 {
		t.Fatalf("expected 1 location, got %d", len(servers[0].locations))
	}
	if servers[0].locations[0].proxyPass != "http://backend:3000" {
		t.Errorf("proxy_pass = %q", servers[0].locations[0].proxyPass)
	}
}

func TestParseNginxConfigSSL(t *testing.T) {
	content := `
server {
    listen 443 ssl;
...
`
	tmpFile, err := os.CreateTemp("", "nginx-ssl-legacy-*.conf")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(tmpFile.Name())
	...
	if servers[0].sslCert != "/etc/ssl/cert.pem" {
		t.Errorf("sslCert = %q", servers[0].sslCert)
	}
	if servers[0].sslKey != "/etc/ssl/key.pem" {
		t.Errorf("sslKey = %q", servers[0].sslKey)
	}
	if servers[0].proxyPass != "http://upstream:8080" {
		t.Errorf("proxyPass = %q", servers[0].proxyPass)
	}
}

func TestConvertNginxToYAMLLegacy(t *testing.T) {
	servers := []nginxServer{
		{
	...
	if !strings.Contains(yaml, "host: static.example.com") {
		t.Errorf("should contain host, got:\n%s", yaml)
	}
	if !strings.Contains(yaml, "type: static") {
		t.Errorf("should contain type: static, got:\n%s", yaml)
	}
	if !strings.Contains(yaml, "root: /var/www/static") {
		t.Errorf("should contain root, got:\n%s", yaml)
	}
}

func TestConvertNginxToYAMLProxy(t *testing.T) {
	servers := []nginxServer{
		{
...
func TestConvertNginxToYAMLSSL(t *testing.T) {
	servers := []nginxServer{
		{
	...
	if !strings.Contains(yaml, "mode: manual") {
		t.Errorf("should contain mode: manual, got:\n%s", yaml)
	}
	if !strings.Contains(yaml, "cert: /path/to/cert.pem") {
		t.Errorf("should contain cert path, got:\n%s", yaml)
	}
	if !strings.Contains(yaml, "key: /path/to/key.pem") {
		t.Errorf("should contain key path, got:\n%s", yaml)
	}
}

func TestConvertNginxToYAMLSSLAutoMode(t *testing.T) {
	servers := []nginxServer{
		{
	...
	if !strings.Contains(yaml, "mode: auto") {
		t.Errorf("should contain mode: auto, got:\n%s", yaml)
	}
}

func TestConvertNginxToYAMLDefaultServerName(t *testing.T) {
	servers := []nginxServer{
		{
			serverName: "_",
	...
	if strings.Count(yaml, "host: example.com") != 2 {
		t.Errorf("expected 2 example.com hosts, got:\n%s", yaml)
	}
}

func TestConvertNginxToYAMLWithLocations(t *testing.T) {
	servers := []nginxServer{
		{
			serverName: "multi.example.com",
			locations: []nginxLocation{
				{path: "/", proxyPass: "http://backend1:3000"},
				{path: "/api", proxyPass: "http://backend2:3001"},
			},
		},
	}
	...
	if !strings.Contains(yaml, "type: proxy") {
		t.Errorf("should be proxy type, got:\n%s", yaml)
	}
	if !strings.Contains(yaml, "address: http://backend1:3000") {
		t.Errorf("should contain first upstream, got:\n%s", yaml)
	}
	if !strings.Contains(yaml, "address: http://backend2:3001") {
		t.Errorf("should contain second upstream, got:\n%s", yaml)
	}
}

func TestHasProxyLocations(t *testing.T) {
	if hasProxyLocations(nil) {
		t.Error("nil should return false")
	}
	if hasProxyLocations([]nginxLocation{}) {
		t.Error("empty should return false")
	}
	if hasProxyLocations([]nginxLocation{{path: "/", proxyPass: ""}}) {
		t.Error("no proxy_pass should return false")
	}
	if !hasProxyLocations([]nginxLocation{{path: "/", proxyPass: "http://backend"}}) {
		t.Error("with proxy_pass should return true")
	}
}

func TestParseNginxConfigMultipleServers(t *testing.T) {
	content := `
server {
    listen 80;
...
`
	tmpFile, err := os.CreateTemp("", "nginx-multi-legacy-*.conf")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(tmpFile.Name())
...
func TestParseNginxConfigEmptyFile(t *testing.T) {
	content := `# Just comments, no server blocks`
	tmpFile, err := os.CreateTemp("", "nginx-empty-legacy-*.conf")
	if err != nil {
		t.Fatal(err)
	}
	defer os.Remove(tmpFile.Name())
