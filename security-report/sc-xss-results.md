...
- Impact escalation: `window.open` is called without `noopener`, so the SVG document retains a same-origin `window.opener` reference to the dashboard and can read its state. Additionally, opening a same-origin auxiliary context copies the opener's `sessionStorage`, which holds the bearer token (`sessionStorage.getItem('uwas_token')`, `web/dashboard/src/lib/api.ts:5`). The attacker script can therefore exfiltrate the admin token / drive the admin API, i.e. full server compromise.
