...
HTTP_CODE=$(curl -fsSL -w "%{http_code}" -o "$TMPDIR/$FILENAME" "$URL" 2>/dev/null || echo "000")
...
verify_release_checksum "$TMPDIR/$FILENAME" "$FILENAME" "$TMPDIR/SHA256SUMS"
ok "Checksum verified"

mv "$TMPDIR/$FILENAME" "$TMPDIR/$BINARY"
