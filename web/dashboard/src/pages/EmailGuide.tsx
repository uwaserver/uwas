...
export default function EmailGuide() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-100">Email Configuration</h1>
          ...
          <tbody className="divide-y divide-[#334155]">
            {records.map((r, i) => (
              <tr key={i} className="bg-[#0f172a] hover:bg-[#1e293b]/50">
                ...
                <td className="px-4 py-3 font-mono text-xs text-slate-300">{r.name}</td>
                <td className="px-4 py-3">
                  <code className="rounded bg-[#1e293b] px-2 py-1 text-xs font-mono text-slate-200 select-all">{r.value}</code>
                ...
                <td className="px-4 py-3">
                  <button onClick={() => copy(r.value, `r-${i}`)} className="rounded p-1 text-slate-500 hover:text-slate-300">
      ...
      <div className="rounded-lg border border-[#334155] bg-[#1e293b] p-5">
        <h2 className="text-sm font-semibold text-slate-300 mb-3">Mail Server Options</h2>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            <div key={opt.name} className="rounded-md bg-[#0f172a] p-3">
              <p className="text-xs font-medium text-slate-300">{opt.name}</p>
              <p className="text-[10px] text-slate-500 mb-2">{opt.desc}</p>
              <code className="block rounded bg-[#1e293b] px-2 py-1 text-[10px] font-mono text-slate-400 select-all">{opt.cmd}</code>
            </div>
          ))}
        </div>
      </div>
