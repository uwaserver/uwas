...
export interface DomainDetail {
  ...
  security?: { blocked_paths: string[] | null; waf: { enabled: boolean; bypass_paths?: string[] | null; rules?: string[] | null }; cloudflare_only?: boolean; rate_limit?: { requests: number; window: string | number; by?: string }; ip_whitelist?: string[] | null; ip_blacklist?: string[] | null; hotlink_protection?: { enabled: boolean; allowed_referers: string[] | null; extensions: string[] | null }; geo_block_countries?: string[] | null; geo_allow_countries?: string[] | null };
...
export const fetchDiskUsage = (domain: string) => api<{ domain: string; bytes: number; human: string; root: string }>(`/api/v1/files/${encodeURIComponent(domain)}/disk-usage`);
